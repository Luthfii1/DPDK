...
app_parse_flow_conf(const char *conf_str)
{
	...
	ret = app_parse_opt_vals(conf_str, ',', 5, vals);
	if (ret < 4 || ret > 5) {
		RTE_LOG(ERR, APP, "pfc %u: invalid number of pfc parameters (%d)\n", nb_pfc, ret);
		return ret;
	}		
	...
	RTE_LOG(INFO, APP, "pfc %u: parse %d values\n", nb_pfc, ret);

	if (ret == 5) {
		pconf->tx_core = (uint8_t)vals[4];
		RTE_LOG(INFO, APP, "pfc %u: RX port = %u, TX port = %u, RX core = %u, WT core = %u, TX core = %u\n",
			nb_pfc, pconf->rx_port, pconf->tx_port, pconf->rx_core, pconf->wt_core, pconf->tx_core);
	}
	else {
		pconf->tx_core = pconf->wt_core;
		RTE_LOG(INFO, APP, "pfc %u: RX port = %u, TX port = %u, RX core = %u, WT core = %u\n",
			nb_pfc, pconf->rx_port, pconf->tx_port, pconf->rx_core, pconf->wt_core);
	}
	...
	while ((opt = getopt_long(argc, argv, "i",
		lgopts, &option_index)) != EOF) {
			...
			case 0:
				...
				if (str_is(optname, "back-core")) {
					back_core = atoi(optarg);
					app_used_core_mask |= (1lu << back_core);
					break;
				}
				break;
