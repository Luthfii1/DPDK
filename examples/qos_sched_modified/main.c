...
app_main_loop(__rte_unused void *dummy)
{
	...
	for (i = 0; i < nb_pfc; i++) {
		...
		if (flow->wt_core == lcore_id) {
			...
			flow->wt_thread.rx_port =  flow->rx_port;
	...
	if (mode == APP_RX_MODE) {
		...
		RTE_LOG(INFO, APP, "Hello wt_ind = %u\n", wt_idx);
		for (i = 0; i < wt_idx; i++) {
			...
			RTE_LOG(INFO, APP,
				"flow %u lcoreid %u: rx port %u, sched + write port %u\n",
					i, lcore_id, wt_confs[i]->rx_port, wt_confs[i]->tx_port);
		}
...
void app_stat_to_file(FILE *fp)
{
	...
	fprintf(fp, "%lu", time);
	fprintf(fp, "\t%u", get_qavg_q(tx_port, 0, 0, 0, 0));
	fprintf(fp, "\t%u", get_qavg_q(tx_port, 0, 0, 1, 0));
	fprintf(fp, "\t%u\n", get_qavg_q(tx_port, 0, 0, 12, 1));
	time++;
}
...
main(int argc, char **argv)
{
	...
	if (interactive) {
		...
		// log_file = fopen("../app_stats.log", "w");
		// fprintf(log_file, "Time\tTC0\tTC1\tTC12\n");
		...
		while(!force_quit) {
			...
			// // sleep(1);
			// // app_stat();
			// app_stat_to_file(log_file);
			// clock_gettime(CLOCK_REALTIME, &end);
			// interval = 10000 - (end.tv_sec - start.tv_sec) * 1000000 - (end.tv_nsec - start.tv_nsec) / 1000;
			// if (interval > 0)
			// 	usleep(interval);
		}
