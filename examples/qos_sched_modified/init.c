...
struct rte_eth_dev_tx_buffer *back_tx_buffer[3] = { NULL };
...
struct qos_sched_port_conf {
	uint8_t is_configured;
	struct rte_sched_port *port;
};

static uint8_t is_port_configured[RTE_MAX_ETHPORTS];
static struct qos_sched_port_conf qos_sched_port_config[RTE_MAX_ETHPORTS];
...
app_init_port(uint16_t portid, struct rte_mempool *mp)
{
	...
	if (portid == back_port_tx[0]) {
		/* Allocate tx buffer */
		back_tx_buffer[0] = rte_zmalloc_socket("back_tx_buffer0",
				RTE_ETH_TX_BUFFER_SIZE(MAX_PKT_TX_BURST), 0,
				rte_eth_dev_socket_id(portid));
		if (back_tx_buffer[0] == NULL)
			rte_exit(EXIT_FAILURE, "Port %d TX buffer 0 allocation error\n",
					portid);

		rte_eth_tx_buffer_init(back_tx_buffer[0], MAX_PKT_TX_BURST);
		printf("TX buffer init... ");
	}
	else if (portid == back_port_tx[1]) {
		/* Allocate tx buffer */
		back_tx_buffer[1] = rte_zmalloc_socket("back_tx_buffer1",
				RTE_ETH_TX_BUFFER_SIZE(MAX_PKT_TX_BURST), 0,
				rte_eth_dev_socket_id(portid));
		if (back_tx_buffer[1] == NULL)
			rte_exit(EXIT_FAILURE, "Port %d TX buffer 1 allocation error\n",
					portid);

		rte_eth_tx_buffer_init(back_tx_buffer[1], MAX_PKT_TX_BURST);
		printf("TX buffer init... ");
	}
	else if (portid == back_port_tx[2]) {
		/* Allocate tx buffer */
		back_tx_buffer[2] = rte_zmalloc_socket("back_tx_buffer2",
				RTE_ETH_TX_BUFFER_SIZE(MAX_PKT_TX_BURST), 0,
				rte_eth_dev_socket_id(portid));
		if (back_tx_buffer[2] == NULL)
			rte_exit(EXIT_FAILURE, "Port %d TX buffer 2 allocation error\n",
					portid);

		rte_eth_tx_buffer_init(back_tx_buffer[2], MAX_PKT_TX_BURST);
		printf("TX buffer init... ");
	}
...
static struct rte_sched_pipe_params pipe_profiles[MAX_SCHED_PIPE_PROFILES] = {
	{ /* Profile #0 */
		...
		.tc_rate = {125000000, 125000000, 125000000, 125000000, 125000000, 125000000,
			125000000, 125000000, 125000000, 125000000, 125000000, 125000000, 125000000},
		.tc_period = 10,
#ifdef RTE_SCHED_SUBPORT_TC_OV
		.tc_ov_weight = 1,
#endif

		.wrr_weights = {20, 1, 1, 1},
	},
};
...
static struct rte_sched_subport_profile_params
		subport_profile[MAX_SCHED_SUBPORT_PROFILES] = {
	{
		.tb_rate = 125000000,
		.tb_size = 12500000,
		.tc_rate = {125000000, 125000000, 125000000, 125000000,
...
struct rte_sched_port_params port_params = {
	...
	.socket = 0, /* computed */
	.rate = 0, /* computed */
	.mtu = 6 + 6 + 2 + 1500, /* Dst MAC + Src MAC + EtherType + Payload */
...
int app_init(void)
{
	...
	if (back_core == -1) {
		rte_exit(EXIT_FAILURE, "No back core assigned\n");
	}
	...
	RTE_LOG(INFO, APP, "Complete cfg file loading with %u pfc\n", nb_pfc);
	...
	for(i = 0; i < nb_pfc; i++) {
		...
		if (qos_conf[i].mbuf_pool == NULL)
			rte_exit(EXIT_FAILURE, "Cannot init mbuf pool for socket %u (err = %d)\n",
				rte_eth_dev_socket_id(qos_conf[i].rx_port), rte_errno);

		printf("PFC %u complete init of mem pool (socket %d)\n", i, rte_eth_dev_socket_id(qos_conf[i].rx_port));
		fflush(stdout);

		if (!is_port_configured[qos_conf[i].rx_port]) {
			app_init_port(qos_conf[i].rx_port, qos_conf[i].mbuf_pool);
			is_port_configured[qos_conf[i].rx_port] = 1;
		}

		if (!is_port_configured[qos_conf[i].tx_port]) {
			app_init_port(qos_conf[i].tx_port, qos_conf[i].mbuf_pool);
			is_port_configured[qos_conf[i].tx_port] = 1;
		}		

		if (!qos_sched_port_config[qos_conf[i].tx_port].is_configured) {
			qos_conf[i].sched_port = app_init_sched_port(qos_conf[i].tx_port, socket);
			qos_sched_port_config[qos_conf[i].tx_port].is_configured = 1;
			qos_sched_port_config[qos_conf[i].tx_port].port = qos_conf[i].sched_port;
		}
		else {
			qos_conf[i].sched_port = qos_sched_port_config[qos_conf[i].tx_port].port;
		}
	}
