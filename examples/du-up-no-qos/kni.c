...
{
	uint8_t i, port_id;
	struct kni_port_params *ue_kni_params;

	/* Invoke rte KNI init to preallocate kni ifaces for UE */
	rte_kni_init(nb_ue_port);

	RTE_LOG(INFO, DU_UP, "Init KNI system with %u KNI devices\n", nb_ue_port);

	for (i = 0; i < nb_ue_port; i++) {
		port_id = ue_ports[i];
		ue_kni_params = rte_zmalloc("KNI_UE_SIDE", sizeof(struct kni_port_params), RTE_CACHE_LINE_SIZE);

		ue_kni_params->port_id = port_id;
		ue_kni_params->group_id = port_id;
		ue_kni_params->nb_lcore_k = 0;
		sprintf(ue_kni_params->kni_device_name, "ue%u", port_id);

		if (kni_alloc_internal(ue_kni_params, pktmbuf_pools[port_id]) < 0) {
			rte_exit(EXIT_FAILURE, "Failed to create UE%u kni device\n", port_id);
		}
		rte_atomic32_init(&kni_pause[ue_kni_params->group_id]);

		kni_port_params_array[port_id] = ue_kni_params;
	}
}
...
kni_alloc_internal(struct kni_port_params *params, struct rte_mempool *pktmbuf_pool)
{
	...
	if (i == 0) {
		...
		RTE_LOG(INFO, DU_UP, "Create KNI interface for port %u\n", params->port_id);
	} else
...
kni_free()
{
	for (int i = 0; i < nb_ue_port; i++) {
		uint16_t port_id = ue_ports[i];

		kni_free_(kni_port_params_array[port_id]);
...
kni_change_mtu_(uint16_t port_id, unsigned int new_mtu)
{
	...
	struct rte_mempool *pktmbuf_pool = pktmbuf_pools[port_id];
...
{
	int ret = 0;
	struct kni_port_params *kni_params = kni_port_params_array[port_id];
	...
	RTE_LOG(INFO, DU_UP, "Configure network interface of vEth-%s %s\n",
					kni_params->kni_device_name, if_up ? "up" : "down");
		...
		for (j = 0; j < num; j++) {
			...
			if (rte_be_to_cpu_16(eth_hdr->ether_type) == RTE_ETHER_TYPE_ARP) {
				...
				if (arp_entry)
					...
					num = rte_eth_tx_burst(p->port_id, 0, &pkt, 1);
			}
