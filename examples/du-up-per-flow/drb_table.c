...
static inline uint32_t hash_function(uint32_t dl_teid)
{
    // return jenkins_one_at_a_time_hash((uint8_t*) &dl_teid, sizeof(dl_teid));
    return dl_teid;
}
...
insert_drb_to_AVL_tree(struct drb_table_entry **tree_root, struct drb_table_entry *drb_entry)
{
    ...
    if (drb_entry->f1u_dl_teid < cp->f1u_dl_teid) {
    ...
    if (cp->avl_balance > 1) {
        // LL Rotation
        if (drb_entry->f1u_dl_teid < cp->left->f1u_dl_teid) {
    ...
    // Unbalance (R/-)
        ...
        // RR Rotation
        if (drb_entry->f1u_dl_teid > cp->right->f1u_dl_teid) {
...
{
    uint32_t dl_teid = drb_context->f1u_dl_teid;
    uint32_t table_idx;
    struct drb_table_entry *entry;

    entry = drb_table_get_entry_by_dl_teid(dl_teid);

    if (entry) {
        RTE_LOG(WARNING, DU_UP, "F1U DL TEID already used for another drb %u\n", entry->drb_context->drb_id);
        return 0;
    }

    table_idx = hash_function(dl_teid) & DRB_HASK_TABLE_INDEX_MASK;
    ...
    entry->f1u_dl_teid = dl_teid;
    entry->drb_context = drb_context;
    entry->avl_balance = 0;
    entry->height = 1;

    // RTE_LOG(INFO, DU_UP, "Insert DRB %u with DL TEID %u into table entry %u\n", drb_context->drb_id, drb_context->f1u_dl_teid, table_idx);
    insert_drb_to_AVL_tree(&drb_hash_table[table_idx], entry);
    // insert_drb_to_AVL_tree(&avl_tree_root, entry);

    return 0;
}

struct drb_table_entry* drb_table_get_entry_by_dl_teid(uint32_t dl_teid)
{
    struct drb_table_entry *cp;
    struct drb_table_entry *entry = NULL;
    uint32_t table_idx = dl_teid & DRB_HASK_TABLE_INDEX_MASK;

    cp = drb_hash_table[table_idx];
    // cp = avl_tree_root;

    while (cp) {
        if (dl_teid == cp->f1u_dl_teid)
            break;
        else if (dl_teid < cp->f1u_dl_teid)
            cp = cp->left;
        else
            cp = cp->right;
    }
...
static void print_avl_tree_preorder(struct drb_table_entry *root) {
    if (root) {
        printf("[DRB %d]: DL TEID = %u, AVL balance = %d, Height = %d\n",
            root->drb_context->drb_id, root->drb_context->f1u_dl_teid, root->avl_balance, root->height);
