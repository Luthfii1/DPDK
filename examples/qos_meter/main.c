...
static uint16_t port_tx = 1;
static struct rte_mbuf *pkts_rx[PKT_RX_BURST_MAX];
struct rte_eth_dev_tx_buffer *tx_buffer;
struct rte_eth_dev_tx_buffer *portrx_tx_buffer;
...
struct rte_meter_trtcm_params app_trtcm_params = {
	.cir = 1000000 * 750,  // 80 M
	.pir = 1000000 * 1000,  // 200 M
	.cbs = 100000 * 125,
	.pbs = 100000 * 125
};
...
app_pkt_handle(struct rte_mbuf *pkt, uint64_t time)
{
	...
	if (output_color == RTE_COLOR_RED){
		// action = DROP;
		action = RED;
		pkt_data[DSCP_BYTE_OFFSET] = RED << 2;
...
main_loop(__rte_unused void *dummy)
{
	...
	while (1) {
		...
		if (unlikely(time_diff > TIME_TX_DRAIN)) {
			...
			rte_eth_tx_buffer_flush(port_rx, NIC_TX_QUEUE, portrx_tx_buffer);
		...
		for (i = 0; i < nb_rx; i ++) {
			...
			if (app_pkt_handle(pkt, current_time) == DROP) {
				...
				rte_eth_tx_buffer(port_rx, NIC_TX_QUEUE, portrx_tx_buffer, pkt);
		}
	}
}
...
parse_args(int argc, char **argv)
{
	...
	while ((opt = getopt_long(argc, argvopt, "p:r:t:", lgopts, &option_index)) != EOF) {
		switch (opt) {
		case 'p':
...
main(int argc, char **argv)
{
	...
	/* Allocate tx buffer */
	portrx_tx_buffer = rte_zmalloc_socket("tx_buffer",
			RTE_ETH_TX_BUFFER_SIZE(PKT_TX_BUFFER_MAX), 0,
			rte_eth_dev_socket_id(port_rx));
	if (portrx_tx_buffer == NULL)
		rte_exit(EXIT_FAILURE, "Port %d TX buffer allocation error\n",
				port_rx);

	rte_eth_tx_buffer_init(portrx_tx_buffer, PKT_TX_BUFFER_MAX);
	...
	rte_eth_tx_buffer_init(tx_buffer, PKT_TX_BUFFER_MAX);
