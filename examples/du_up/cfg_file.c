...
static int cfg_load_lcore(struct rte_cfgfile *cfg)
{
    ...
    for (lcore_id = 0; lcore_id < APP_MAX_LCORE; lcore_id++) {
        ...
        if (strcmp(entry, "ue") == 0)
            conf->role = LCORE_UE;
        else if (strcmp(entry, "cu_tx") == 0) {
            conf->role = LCORE_CU_TX;
        }
...
static int cfg_load_up_setting(struct rte_cfgfile *cfg)
{
    ...
    if (!rte_cfgfile_has_section(cfg, "CU_UP")) {
        RTE_LOG(ERR, DU_UP, "Cfgfile: must specify CU_UP in cfg file\n");
        return -1;
    }
    ...
    // CU_UP
    entry = rte_cfgfile_get_entry(cfg, "CU_UP", "ip");
    if (!entry) {
        RTE_LOG(ERR, DU_UP, "Cfgfile: No CU_UP ip is provided\n");
        return -1;
    }
    if (inet_pton(AF_INET, entry, &cu_up_ip) <= 0) {
        RTE_LOG(ERR, DU_UP, "Cfgfile: CU_UP ip is invalide\n");
        return -1;
    }
    RTE_LOG(INFO, DU_UP, "CU_UP ip is %s\n", entry);
